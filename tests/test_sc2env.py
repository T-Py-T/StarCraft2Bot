...
def test_step_waits_for_ready_correlated_response(monkeypatch) -> None:
    ...
    assert writes[0]["episode_id"] == "episode-1"
    assert writes[0]["request_id"] == 1
...
def test_step_timeout_invalidates_episode_and_stops_child(monkeypatch) -> None:
    ...
    assert writes[0]["episode_id"] == "episode-1"
    assert writes[0]["request_id"] == 1
    assert writes[1]["episode_id"] != "episode-1"
...
def test_reset_restarts_child_for_each_episode(monkeypatch) -> None:
    ...
    assert writes[0]["episode_id"] != writes[1]["episode_id"]
    assert starts[0][1]["SC2_EPISODE_ID"] == writes[0]["episode_id"]
    assert starts[1][1]["SC2_EPISODE_ID"] == writes[1]["episode_id"]
